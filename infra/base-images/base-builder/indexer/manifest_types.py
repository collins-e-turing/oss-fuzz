...
class SourceRef:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SourceRef":
        ...
        return cls(url=data["url"], rev=data["rev"], type=RepositoryType(data["type"]))
  ...
  def from_dict(cls, data: dict[str, Any]) -> Self:
    ...
    if data["version"] < 4:
      ...
      binary_config = _get_mapped(data, "binary_config", BinaryConfig.from_dict)
        ...
        if binary_config is None or not isinstance(binary_config, BinaryConfig):
            raise RuntimeError(
                "Manifest construction error: binary_config must be a valid BinaryConfig instance, got {}".format(
                    type(binary_config)
                )
            )
        return Manifest(
    ...
    def validate(self) -> None:
        ...
        if isinstance(binary_config, CommandLineBinaryConfig):
    ...
    if os.path.exists(archive_path) and not overwrite:
      raise FileExistsError(f"Not overwriting existing archive {archive_path}")
        ...
        if not (
            hasattr(self.binary_config, "binary_name")
            and isinstance(self.binary_config, CommandLineBinaryConfig)
        ):
            raise RuntimeError(
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            with tarfile.open(tmp.name, mode) as tar:
                ...
                def _save_dir(
                    ...
                    # Ensure prefix remains a Path object; use as_posix() for string operations
                        ...
                        for file in files:
                            ...
                            if only_include_target and _is_elf(file):
                                ...
                                if (
                                    ...
                                    and not file.absolute().is_relative_to(
                                        out_dir / "lib"
                                    )
                                ):
                                    continue
                            ...
                            tar.add(
                                ...
                                arcname=prefix_str + str(file.relative_to(path)),
                            )
...
def report_missing_source_files(
    ...
    copied_files_set = set(_get_comparable_path(file) for file in copied_files)
    try:
        covered_files = {
            _get_comparable_path(path): path for path in get_covered_files(binary_name)
        }
    except Exception as e:
        logging.warning(f"Could not fetch coverage files for {binary_name}: {e}")
        covered_files = {}
    missing = set(covered_files.keys()) - copied_files_set
    if not missing:
        return
    ...
    try:
        out_path = (
            ...
            else os.path.join(OUT, report_name)
        )
        with open(str(out_path), "w") as fp:
            fp.write(missing_report)
    except Exception as e:
        logging.warning(f"Could not write missing report to OUT: {e}")
