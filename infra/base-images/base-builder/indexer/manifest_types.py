...
class BinaryConfig:
  ...
  @classmethod
  def from_dict(cls, config_dict: Mapping[str, Any]) -> BinaryConfig:
...
class CommandLineBinaryConfig(BinaryConfig):
  ...
  @classmethod
  def from_dict(cls, config_dict: Mapping[str, Any]) -> CommandLineBinaryConfig:
...
class Manifest:
  ...
  def from_dict(cls, data: dict[str, Any]) -> Manifest:
    ...
    return Manifest(
        ...
        binary_config=binary_config or BinaryConfig(kind=BinaryConfigKind.BINARY),
    )
  ...
  def validate(self) -> None:
    ...
    if isinstance(self.binary_config, CommandLineBinaryConfig):
      if not isinstance(self.binary_config.binary_args, list):
        raise RuntimeError(
            "Type mismatch for field binary_config.binary_args: expected list, "
            f"got {type(self.binary_config.binary_args)}"
        )
  ...
  def save_build(
    ...
    if isinstance(self.binary_config, CommandLineBinaryConfig):
      binary_name = self.binary_config.binary_name
    else:
      raise RuntimeError("binary_name is not accessible for this BinaryConfig type.")
    ...
    with tempfile.NamedTemporaryFile() as tmp:
      ...
      with tarfile.open(tmp.name, mode) as tar:
        ...
        if self.index_db_version is None:
          index_db_version = _get_sqlite_db_user_version(Path(index_dir / INDEX_DB))
...
def report_missing_source_files(
  ...
  copied_files = ["/".join(file) for file in copied_files]
  covered_files = {
      _get_comparable_path(path): path
      for path in get_covered_files(binary_name)
  }
  missing = {"/".join(file) for file in covered_files.keys()} - set(copied_files)
  if not missing:
    return
  logging.info("Reporting missing files: %s", missing)
  missing_report_lines = sorted([
      covered_files[(k.split("/", maxsplit=1)[0], k.split("/", maxsplit=1)[1])] 
      for k in missing if len(k.split("/", maxsplit=1)) == 2
  ])
